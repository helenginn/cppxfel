...
for i in range(skip, num_images):
	...
	partitioned = numpy.zeros(shape=(width, height), dtype=numpy.int32)
	
	for j in range(height):
		for k in range(width):
			partitioned[j][k] = picklePixels[j * width + k]
	
	sample['DATA'] = scitbx_array_family_flex_ext.int(partitioned)
